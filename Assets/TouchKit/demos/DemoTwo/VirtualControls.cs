...
public class VirtualControls
{
	...
	private TKAnyTouchRecognizer _jumpRecognizer;
	...
	public GameObject moveFrontButton;
	...
	public VirtualControls()
	{
		...
		{
			buttonHeight *= Camera.main.aspect / 1.5f;
			buttonWidth *= Camera.main.aspect / 1.5f;
		}

		//moveFrontButton = GameObject.FindGameObjectWithTag ("FrontButton");
		//moveFrontButton.renderer.bounds.size;
		setupRecognizers();
	...
	{
		createQuadButton( leftRect, Color.clear );
		createQuadButton( rightRect, Color.white );
		createQuadButton( upRect, Color.clear );

		createQuadButton( attackRect, Color.clear );
		createQuadButton( jumpRect, Color.clear );
	}
	...
	{
		// left button
		_leftRecognizer = new TKAnyTouchRecognizer( new TKRect( buttonWidth, 0f, buttonWidth, buttonHeight ) );
		TouchKit.addGestureRecognizer( _leftRecognizer );


		// right button
		_rightRecognizer = new TKAnyTouchRecognizer( new TKRect( buttonWidth *2f + 5f, 0f, buttonWidth, buttonHeight ) );
		...
		// jump button
		_jumpRecognizer = new TKAnyTouchRecognizer( new TKRect( TouchKit.instance.designTimeResolution.x - buttonWidth, 0, buttonWidth, buttonHeight ));
